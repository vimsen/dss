class MarketPricesController < ApplicationController
    ...
    def intraDay
	 ...
	 mi2_prices = IntraDayEnergyPrice.order("dayhour").where(date: selected_date, market_id: params[:id], interval_id: 2)
         mi3_prices = IntraDayEnergyPrice.order("dayhour").where(date: selected_date, market_id: params[:id], interval_id: 3)
	 mi4_prices = IntraDayEnergyPrice.order("dayhour").where(date: selected_date, market_id: params[:id], interval_id: 3)
	...
	mi4_prices.each do |price|
         ...
         chartData.push({:data => mi1_priceData, :label => "Energy Price Per Hour - MI1"},{:data => mi2_priceData, :label => "Energy Price Per Hour - MI2"},{:data => mi3_priceData, :label => "Energy Price Per Hour - MI3"},{:data => mi4_priceData, :label => "Energy Price Per Hour - MI4"} )
